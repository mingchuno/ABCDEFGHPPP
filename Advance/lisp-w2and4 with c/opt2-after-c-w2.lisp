...
(defun print_met_cond_at_base_dolist_p=1 (base) 
   ...
   (setf *111* (* *p* (+ (* base base) base 1)))
   (dolist (h (number_seq-m 0 (1- base) )) 
    (dolist (g (number_seq-m-not-1 2 (1- base) 1)) 
     (let* ((ef (- *111* (+ (* base g) h)))
            (f (rem ef base))
            (e (/ (- ef f) base)))
         (dolist (d (number_seq-m 0 (1- base)  ))  
          (dolist (c (number_seq-m-not-1 2 (1- base)  1))   
            (let* ((ab (+ ef (+ (* base c) d)))
                   (b  (rem ab base))
                   (a  (/ (- ab b) base)))
              ;;(format t "*111* is ~d -- not solution ~d : ~d~~~d - ~d~~~d = ~d~~~d + ~d~~~d = ~d~~~d~~~d (base ~d)~%" 
              ;;       *111* *no_of_solutions* a b c d e f g h *p* *p* *p* base)
              (when (and (< a  base) 
