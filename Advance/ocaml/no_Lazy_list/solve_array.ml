...
*)
  ...
  let iterate_solution_from_lkji = function
    | [] -> assert false
    | l::kji ->
      let unused = Array.create ~len:base true in
      let () =
        set unused 1 false;
        set unused l false; set unused (base + 1 - l) false;
        List.iter kji ~f:(fun k -> set unused k false; set unused (base - k) false) in 
      let rec go_deeper carry partial_solution = function
        | [i] ->
          let digit_difference = i + if carry then 1 else 0 in
          for e = 2 to biggest_digit - digit_difference do
            let a = e + digit_difference in
              if get unused e && get unused a then new_sol (e::partial_solution)
          done
        | k::ji ->
          let digit_difference = k + if carry then 1 else 0 in
          ...
          let check_and_go_deeper carry g = 
            ...
            if get unused g && get unused c then
              ( set unused g false; set unused c false;
                go_deeper carry (g::partial_solution) ji;
                set unused g true; set unused c true;
              ) in
